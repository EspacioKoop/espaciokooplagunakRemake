#!/usr/bin/env python3
"""Capture eleven real application views using an isolated Xvfb display."""
import argparse
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="lagunak-display-") as temporary:
        ...
        for filename in ["01_inicio", "02_puente", "03_ingenieria", "04_cubierta", "05_reactor", "06_atlas", "07_campana", "08_editor", "09_operaciones", "10_asistencia", "11_astillero"]:
            data = (args.output / (filename + ".png")).read_bytes()
            assert data[:8] == b"\x89PNG\r\n\x1a\n" and struct.unpack(">II", data[16:24]) == (1600, 900), filename
        print("Verified eleven real screenshots at 1600×900.")
