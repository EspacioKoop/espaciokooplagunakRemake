...
var _lounge_identity = "self"
var _resume_lounge_ticket = ""
var _resume_lounge_address = ""
var view: Dictionary = {}
var mode = "offline"
var role = "mando"
var paused = true
...
func host_session(port: int = DEFAULT_PORT, key: String = "") -> Dictionary:
	...
	_lounge_peers = {1: "self"}
	_lounge_tickets.clear()
...
func join_session(address: String, port: int, key: String, player_name: String, requested_role: String) -> Dictionary:
	...
	var endpoint = address.strip_edges() + ":" + str(port) + ":" + key.sha256_text()
	if endpoint != _resume_lounge_address: _resume_lounge_ticket = ""
	_resume_lounge_address = endpoint
	close_session()
	var peer = ENetMultiplayerPeer.new()
...
func close_session() -> void:
	if mode == "host": lounge.cancel_all()
	_lounge_identity = "self"
...
func _challenge(nonce: String, protocol: int) -> void:
	...
	_authenticate.rpc_id(1, proof, _player_name, _desired_role, _resume_lounge_ticket)

@rpc("any_peer", "call_remote", "reliable", 0)
func _authenticate(proof: PackedByteArray, player_name: String, requested_role: String, resume_ticket: String) -> void:
	if mode != "host": return
	...
	if Time.get_ticks_msec() > challenge.until or proof.size() != 32 or not Crypto.new().constant_time_compare(expected, proof):
		...
		return
	if requested_role not in Catalog.ROLES or player_name.length() > 32 or resume_ticket.length() > 64:
	...
	var ticket = resume_ticket
	var identity = str(_lounge_tickets.get(ticket, ""))
	if identity.is_empty() or identity in _lounge_peers.values():
		ticket = Crypto.new().generate_random_bytes(24).hex_encode()
		identity = "crew_" + Crypto.new().generate_random_bytes(8).hex_encode()
		_lounge_tickets[ticket] = identity
	_lounge_peers[id] = identity
	lounge.presence(identity, true)
	_accepted.rpc_id(id, requested_role, ticket, identity)
	_refresh_view()
...
@rpc("authority", "call_remote", "reliable", 0)
func _accepted(assigned_role: String, ticket: String = "", identity: String = "") -> void:
	if not ticket.is_empty(): _resume_lounge_ticket = ticket
	if not identity.is_empty(): _lounge_identity = identity
	role = assigned_role
...
func _peer_disconnected(id: int) -> void:
	if _lounge_peers.has(id): lounge.presence(_lounge_peers[id], false)
	_lounge_peers.erase(id)
	roster.erase(id)
	poses.erase(id)
	_challenges.erase(id)
	_rates.erase(id)
	...
	if _broadcast_clock >= 0.1:
		...
		if mode == "host" and not view.is_empty():
			for id in roster:
				...
				recipient_view.lounge = lounge.snapshot(str(_lounge_peers.get(int(id), "")), roster[id].role == "mando")
...
func table_order(operation: String, parameters: Dictionary) -> Dictionary:
	var args = parameters.duplicate()
	var current: Dictionary = view.get("lounge", {})
	var table: Dictionary = current.get("tables", {}).get(str(args.get("table", "")), {})
	args.epoch = current.get("epoch", "")
	args.revision = table.get("revision", -1)
	args.nonce = Crypto.new().generate_random_bytes(12).hex_encode()
	if mode == "client":
		_receive_table.rpc_id(1, operation, args)
		return {"ok": true, "message": "Jugada enviada al anfitrión."}
	var response = lounge.command("self", "Anfitrión" if mode == "host" else "Tú", role == "mando", operation, args)
	notice.emit(response.message, response.ok)
	_refresh_view()
	return response

@rpc("any_peer", "call_remote", "reliable", 0)
func _receive_table(operation: String, args: Dictionary) -> void:
	if mode != "host" or not operation.begins_with("table_"): return
	var id = multiplayer.get_remote_sender_id()
	if not roster.has(id) or not _lounge_peers.has(id) or not _allow_order(id): return
	var response = lounge.command(_lounge_peers[id], roster[id].name, roster[id].role == "mando", operation, args)
	_response.rpc_id(id, response.message, response.ok)
	_refresh_view()
