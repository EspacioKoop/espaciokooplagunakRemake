...
func _rebuild() -> void:
	if tabs == null or systems == null: return
...
func _make_atlas() -> void:
	...
	if systems.data.atlas.sectors.is_empty(): sectors.add_child(ConsoleUI.paragraph("Todavía no hay sectores registrados.", 15))
	...
	var label = LineEdit.new(); label.placeholder_text = "Nombre del marcador"; label.text = "Punto de interés"; label.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(label)
	var x = SpinBox.new(); x.min_value = -12000; x.max_value = 12000; x.step = 10; row.add_child(x)
	var y = SpinBox.new(); y.min_value = -12000; y.max_value = 12000; y.step = 10; row.add_child(y)
	...
	for item in systems.data.atlas.markers:
		...
		if item.author == systems.actor_id(): line.add_child(ConsoleUI.button("Quitar", func(id = item.id): systems.command("marker_remove", {"id": id})))
...
func _make_factions() -> void:
	...
	for id in ExpeditionSystems.COMMODITIES:
		...
		label.size_flags_horizontal = Control.SIZE_EXPAND_FILL; line.add_child(label)
		var buy = ConsoleUI.button("Comprar 1", func(item = id): systems.command("trade_buy", {"item": item, "amount": 1})); buy.disabled = not docked; line.add_child(buy)
		var sell = ConsoleUI.button("Vender 1", func(item = id): systems.command("trade_sell", {"item": item, "amount": 1})); sell.disabled = not docked or int(stock[id]) < 1; line.add_child(sell)
...
func _make_crew() -> void:
	...
	var name = LineEdit.new(); name.text = profile.name; name.max_length = 32; card.add_child(name)
	var approach = OptionButton.new()
	for value in ExpeditionSystems.APPROACHES: approach.add_item(value.capitalize()); approach.set_item_metadata(approach.item_count - 1, value)
	for i in approach.item_count:
		if approach.get_item_metadata(i) == profile.approach: approach.select(i)
	card.add_child(approach)
	var skill_values = {}
	for skill in ExpeditionSystems.SKILLS:
		var row = ConsoleUI.row(card)
		var label = ConsoleUI.label(skill.capitalize(), 15); label.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(label)
		var value = SpinBox.new(); value.min_value = 0; value.max_value = 4; value.step = 1; value.value = int(profile.skills.get(skill, 1)); row.add_child(value); skill_values[skill] = value
	var save = ConsoleUI.button("Guardar ficha", func():
		...
		for skill in ExpeditionSystems.SKILLS: args[skill] = int(skill_values[skill].value)
...
func _skill_total(skills: Dictionary) -> int:
	var total = 0
	for value in skills.values(): total += int(value)
	return total
...
func _make_chronicle() -> void:
	...
	if systems.data.bestiary.is_empty(): bestiary.add_child(ConsoleUI.paragraph("Identifica contactos para alimentar el catálogo.", 15))
...
func _make_director() -> void:
	...
	var tempo = HSlider.new(); tempo.min_value = 0.25; tempo.max_value = 3.0; tempo.step = 0.05; tempo.value = float(systems.data.director.tempo); tempo.custom_minimum_size.y = 38; tempo_card.add_child(tempo)
	var tempo_label = ConsoleUI.label("Tempo %.2f" % tempo.value, 15, ConsoleUI.TEAL); tempo_card.add_child(tempo_label); tempo.value_changed.connect(func(value): tempo_label.text = "Tempo %.2f" % value)
	var apply_tempo = ConsoleUI.button("Aplicar tempo", func(): systems.command("director_tempo", {"value": tempo.value})); apply_tempo.disabled = not can_direct; tempo_card.add_child(apply_tempo)
	var automatic = CheckBox.new(); automatic.text = "Encuentros automáticos cada ciclo de amenaza"; automatic.button_pressed = bool(systems.data.director.auto_events); automatic.disabled = not can_direct; automatic.toggled.connect(func(value): systems.command("director_auto", {"enabled": value})); tempo_card.add_child(automatic)
	var threat = ProgressBar.new(); threat.max_value = 90; threat.value = float(systems.data.director.threat); threat.custom_minimum_size.y = 12; tempo_card.add_child(threat)
	var spawn = ConsoleUI.card(root, "CONVOCAR ENCUENTRO")
	var row = ConsoleUI.row(spawn)
	var kind = OptionButton.new(); for value in ExpeditionSystems.DIRECTOR_KINDS: kind.add_item(value.capitalize()); kind.set_item_metadata(kind.item_count - 1, value); row.add_child(kind)
	var name = LineEdit.new(); name.text = "Patrulla Itzal"; name.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(name)
	var x = SpinBox.new(); x.min_value = -3500; x.max_value = 3500; x.value = 900; x.step = 50; row.add_child(x)
	var y = SpinBox.new(); y.min_value = -3500; y.max_value = 3500; y.step = 50; row.add_child(y)
	var create = ConsoleUI.button("Convocar", func(): systems.command("director_spawn", {"kind": kind.get_item_metadata(kind.selected), "name": name.text, "x": x.value, "y": y.value}), true); create.disabled = not can_direct; row.add_child(create)
	var supply = ConsoleUI.button("Reposición de emergencia · 25 créditos", func(): systems.command("director_supply", {})); supply.disabled = not can_direct; spawn.add_child(supply)
