...
static func run(runner: SceneTree) -> void:
	...
	for index in deck._corridors.doors.size():
		...
		var crossed: bool = await preload("res://release_acceptance/fixtures/test_ship_corridors.gd").walk(runner, door.position + normal * 0.8, "open hatch crossing %d" % index)
